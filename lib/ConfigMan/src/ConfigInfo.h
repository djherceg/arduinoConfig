...
public:
    ...
    ConfigInfo(uint8_t pincount, uint16_t vertime16, uint16_t verdate16, uint8_t vermajor, uint8_t verminor, const char *devicename_P);
    ...
    uint8_t getPinCount() { return this->pinCount; };
    ...
    int processCommand(uint8_t *cmd, int8_t cmdlen, uint8_t *dest, uint8_t destlen);

    /** Obrađuje 'SET' poruku u kojoj se nalaze zadate vrednosti pinova.*/
    int processSet(uint8_t *buffer, uint8_t length);
    ...
    int processQuery(uint8_t *message, uint8_t msglen, uint8_t *buffer, uint8_t buflen);

    /** Obrađuje 'INFO' poruku u message i smešta odgovor u buffer dužine buflen. 
        Vraća dužinu odgovora kao rezultat ili 0 ako ne uspe. */
    int processInfo(uint8_t *message, uint8_t msglen, uint8_t *buffer, uint8_t buflen);

    /** Obrađuje 'VERSION' poruku u message i smešta odgovor u buffer dužine buflen. 
        Vraća dužinu odgovora kao rezultat ili 0 ako ne uspe. */
    int processVersion(uint8_t *message, uint8_t msglen, uint8_t *buffer, uint8_t buflen);
};
